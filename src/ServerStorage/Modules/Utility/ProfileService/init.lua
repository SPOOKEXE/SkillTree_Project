...
function Profile:Save()
	...
	if IsCustomWriteQueueEmptyFor(self._profile_store._profile_store_lookup, self._profile_key) == true then
		...
		task.defer(SaveProfileAsync, self)
	end
end
...
function Profile:Release()
	...
	if self:IsActive() == true then
		task.defer(SaveProfileAsync, self, true) -- Call save function in a new thread with release_from_session = true
	end
end
...
RunService.Heartbeat:Connect(function()
	...
	if auto_save_list_length > 0 then
		...
		while os_clock - LastAutoSave > auto_save_index_speed do
			...
			-- Perform save call:
			if profile ~= nil then
				task.defer(SaveProfileAsync, profile) -- Auto save profile in new thread
			end
		end
	end
	-- 2) Issue queue: --
		...
		function()
			...
			for _, profile in ipairs(active_profiles) do
				if profile:IsActive() == true then
					...
					task.defer(function() -- Save profile on new thread
